...
module_param(gpioLED, uint,
						 S_IRUGO); ///< Param desc. S_IRUGO can be read/not changed
MODULE_PARM_DESC(gpioLED,
								 " GPIO LED number (default=17)"); ///< parameter description
...
enum modes
{
	...
	ON,
	FLASH
}; ///< The available LED modes -- static not useful here
 ...
 *  @return return the number of characters of the mode string successfully
 * displayed
 */
static ssize_t
...
static ssize_t
mode_store(struct kobject* kobj,
...
static ssize_t
period_show(struct kobject* kobj, struct kobj_attribute* attr, char* buf)
{
...
static ssize_t
period_store(struct kobject* kobj,
						 struct kobj_attribute* attr,
						 const char* buf,
						 size_t count)
{
	...
	if ((period > 1) &&
			(period <= 10000)) { // Must be 2ms or greater, 10secs or less
 ...
 * show and store function pointers The period variable is associated with the
 * blinkPeriod variable and it is to be exposed with mode 0666 using the
 * period_show and period_store functions above
 */
static struct kobj_attribute period_attr =
	__ATTR(blinkPeriod, 0664, period_show, period_store);
 ...
 * attribute group below. The attr property of the kobj_attribute is used to
 * extract the attribute struct
 */
 ...
 * sysfs -- in this case it is gpio17, which is automatically defined in the
 * erpi_LED_init() function below using the custom kernel parameter that can be
...
flash(void* arg)
{
	...
	if (mode == FLASH)
		...
		ledOn = false;
	gpio_set_value(gpioLED,
								 ledOn); // Use the LED state to light/turn off the LED
 ...
 * its memory freed up after that point. In this example this function sets up
 * the GPIOs and the IRQ
 *  @return returns 0 if successful
 */
static int __init
erpi_LED_init(void)
{
	...
	erpi_kobj = kobject_create_and_add(
		"erpi", kernel_kobj->parent); // kernel_kobj points to /sys/kernel
	if (!erpi_kobj) {
	...
	gpio_direction_output(gpioLED,
												ledOn); // Set the gpio to be in output mode and turn on
	gpio_export(
		gpioLED,
		false); // causes gpio17 to appear in /sys/class/gpio
						// the second argument prevents the direction from being changed

	task = kthread_run(
		flash, NULL, "LED_flash_thread"); // Start the LED flashing thread
