...
static void handle_get(int connection_fd, const char *page)
{
	...
	// Make sure the requested page begins with a slash and does no contain any additional slashes
	// - we don't support any subdirectories
	...
	if (module == NULL)
	{
		// Either the requested page was malformed, or we couldn't open a module with the indicated name.
		// Either way, return the HTTP response 404, Not Found
...
static void handle_connection(int connection_fd)
{
	...
	if (bytes_read < 0)
	{
		// The call to read failed - print error and exit
		system_error("read");
	}
	...
	if (bytes_read > 0)
	{
		...
		// The first line the client sends is the HTTP request, which is composed of a method, the requested
		// page, and the protocol version
		sscanf(buffer, "%s %s %s", method, url, protocol);

		// The client may send various header information following the request. For this HTTP implementation,
		// we don't care about it. However, we need to read any data the client tries to send. Keep on reading
		// data until we get to the end of the header, which is delimited by a blank line. HTTP specifies
		...
		{
			// This server only implements the GET method. The client specified some other method, so report failure
			char response[1024];
...
void server_run(struct in_addr local_address, uint16_t port)
{
	...
	int rval = bind(server_socket, (struct sockaddr *)&socket_address, sizeof(socket_address));
	if (rval != 0)
		system_error("bind");

	// Instruct the socket to accept connectionss
	...
	while (1)
	{
		...
		if (verbose)
		{
			...
			socklen_t addr_length = sizeof(socket_address);
			rval = getpeername(connection, (struct sockaddr *)&socket_address, &addr_length);
			assert(rval == 0);
		...
		{
			// This is the parent process. The child process handles the connection, so we don't need our
			// copy of the connected socket descriptor. Close it. Then continue with the loop and accept
			// another connection
			close(connection);
		}
		else
