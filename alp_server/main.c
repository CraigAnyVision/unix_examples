...
static const struct option long_options[] = {
    {"address", 1, NULL, 'a'}, {"help", 0, NULL, 'h'},    {"module-dir", 1, NULL, 'm'},
    {"port", 1, NULL, 'p'},    {"verbose", 0, NULL, 'v'},
};
...
int main(int argc, char* const argv[])
{
	...
	do
	{
		...
		switch (next_option)
		{
			case 'a':  // User specified -a or --address
			{
			...
			case 'h':  // User specified -h or --help
				print_usage(0);

			case 'm':  // User specified -m or --module-dir
			{
			...
			case 'p':  // User specified -p or --port
			{
				char* end;
				uint16_t value = (uint16_t)strtol(optarg, &end, 10);
				if (*end != '\0')
				{
			...
			case 'v':  // User specified -v or --verbose
				verbose = 1;
				break;
