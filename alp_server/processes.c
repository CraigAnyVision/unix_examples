...
// Set *UID and *GID to the owning user ID and group ID, respectively, of process PID
...
// Return the name of the program running in process PID, or NULL on error.  The return value
// is a newly allocated buffer which the caller must deallocate with free
static char *get_program_name(pid_t pid)
{
	...
	char *result = (char *)xmalloc(close_paren - open_paren);
...
// Generate an HTML table row for process PID.  The return value is a pointer to a buffer that
// the caller must deallocate with free, or NULL if an error occurs
static char *format_process_info(pid_t pid)
{
	...
	size_t result_length = strlen(program_name) + strlen(user_name) + strlen(group_name) + 128;
	char *result = (char *)xmalloc(result_length);
...
{
	// Set up an iovec array.  We’ll fill this with buffers that’ll be part of our output, growing
	// it dynamically as necessary
	...
	while (1)
	{
		...
		char *process_info = format_process_info(pid);
		if (process_info == NULL)
		{
			// Something went wrong.  The process may have vanished while we were looking at it
			// Use a placeholder row instead
			process_info = "<tr><td colspan=\"5\">ERROR</td></tr>";
		}

		// Make sure the iovec array is long enough to hold this buffer (plus one more because we’ll add an
		// extra element when we’re done listing processes). If not, grow it to twice its current size
